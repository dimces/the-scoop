...
function updateComment(url, request) {
  ...
  //console.log(`>>> REQUEST: ${pp( request )}`);
  //console.log(`>>> RESPONSE: ${pp( response )}`);
  //console.log(`>>> requestComment: ${pp( requestComment )}`);
  //console.log(`>>> databaseCommID: ${pp( database.comments[id] )}`);
...
function upVoteComment(url, request) {
  ...
  if (savedComment && database.users[commentUsername]) {
    savedComment = upvote(savedComment, commentUsername);
...
function downVoteComment(url, request) {
  ...
  if (savedComment && database.users[commentUsername]) {
    savedComment = downvote(savedComment, commentUsername);
...
function downvote(item, username) {
  ...
  /*const upVotedArray = database.comments[id].upvotedBy;
  const downVotedArray = database.comments[id].downvotedBy;
  const downVotedIndex = downVotedArray.indexOf(commentUsername);
  console.log(`>>> REQUEST: ${pp(request)}`);
  function checkUpvoted (username) {
    return upVotedArray.includes(username);
  };
  function checkDownvoted (username) {
    return downVotedArray.includes(username);
  };
  function validUserName (userName) {
    const usersArray = Object.keys(database.users);
    return usersArray.includes(userName);
  };
  function badId(id) {
    return database.comments[id] === null || database.comments[id] === undefined;
  };

  if ( !checkUpvoted(commentUsername) && !checkDownvoted(commentUsername) 
  && validUserName(commentUsername) && commentUsername ) {
    upVotedArray.push(commentUsername);
    response.status = 200;
    response.body = {
  ...
  if (!checkUpvoted(commentUsername) && checkDownvoted(commentUsername) 
  && validUserName(commentUsername) && commentUsername ) {
    downVotedArray.splice(downVotedIndex, 1);
    upVotedArray.push(commentUsername);
    response.status = 200;
    response.body = {
      comment: database.comments[id]
  ...
  if ( !validUserName(commentUsername) ) {
    //console.log('sucseeees ' );
    response.status = 400;
  } 
  if ( badId(id) ) {
    //console.log( 'Here it is ' + badId(id ) )
    response.status = 400;
  }
  //console.log(`>>> comment: ${pp(request.body)}`);
  //console.log(`>>> EVE: ${pp( database.users )}`);
  //console.log(`>>> comment: ${pp( database.users[commentUsername])}`);
  //console.log(`>>> commentUserna: ${pp( database.users[commentUsername].username)}`);
  //console.log(`>>> requestBody: ${pp( request.body)}`);
  //console.log(`>>> requestBodyUser: ${pp( request.body.username)}`);
  //console.log( 'eve go ' + validUserName(commentUsername) );
